...
void afficherLoading(SDL_Renderer * renderer, SDL_Texture * loadingTexture, SDL_Color color, int shiftX, int shiftY, int frameAnim, int windowW, int windowH){
	if(frameAnim){
		SDL_SetTextureAlphaMod(loadingTexture, 255);
		SDL_Rect dest = {windowW - 3/2. * LOADING_SIZE + shiftX , windowH - 3/2. * LOADING_SIZE + shiftY, LOADING_SIZE, LOADING_SIZE};
...
void afficherRetour(SDL_Renderer * renderer, SDL_Texture * loadingTexture,TTF_Font * font, SDL_Color color, int shiftX, int shiftY, int frameAnim, int windowW, int windowH){
	int fLogo= frameAnim;
	if(fLogo < 0){
