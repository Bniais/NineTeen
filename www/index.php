<?php 
	...
	if ( $resultat = $link->query("SELECT username, SUM(score) AS total FROM nineteen_scores NATURAL JOIN nineteen_players GROUP BY userId ORDER BY total DESC LIMIT 3") )
		{
			$indice = 0;
			while ($row = $resultat->fetch_assoc() ) {
