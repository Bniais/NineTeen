...
*/
	...
	Enemy* findBoss(Enemy *enemies, int nbEnemy){
		for(int i = 0; i<nbEnemy; i++){
			if( enemies[i].id == ARM_BOSS_ENEMY) // isboss ?
				return &(enemies[i]);
		}
		return NULL;
	}
...
*/
	...
	// Test for collision between an ellipse of horizontal radius w0 and vertical radius h0 at (x0, y0) and an ellipse of horizontal radius w1 and vertical radius h1 at (x1, y1)
	...
	// Test for collision between two ellipses, "0" and "1". Ellipse is at (x, y) with major or minor radius vector (wx, wy) and the other major or minor radius perpendicular to that vector and hw times as long.
	int collideOblique(double x0, double y0, double wx0, double wy0, double hw0,double x1, double y1, double wx1, double wy1, double hw1, int collideRect) {
...
*/
	...
	void drawWarning(SDL_Renderer *renderer, int frameBossBar, SDL_Texture **textures){
		if(frameBossBar > FRAME_BAR_SPAWN-NB_PERIODE*PERIODE_CLIGNOTE){
			SDL_Rect src = BOSS_BAR_SRC;
			SDL_Rect dest = BOSS_BAR_DEST;
			SDL_SetTextureAlphaMod(textures[SH_WARNING], 255*fabs(cos((2*PI*frameBossBar)/(2*PERIODE_CLIGNOTE) )) );
			SDL_RenderCopy(renderer, textures[SH_WARNING], &src, &dest);
		}
	}

	void drawBossBar(SDL_Renderer *renderer, int frameBossBar, Enemy *enemy, SDL_Texture **textures, float ratioWindowSize){
		float ratio;
		if (frameBossBar > 0){//appear
			ratio = (FRAME_APPARITION_BAR-frameBossBar)/(float)TIME_APPARITION_BAR;
		}
		else if(frameBossBar == 0 && enemy){// alive
			ratio = enemy->hp / ENEMY_HP[enemy->id];
		}
		else {//disappear
			ratio = 0;
		}
		...
		//BarContent
		if(ratio){
			SDL_SetRenderDrawColor(renderer, BOSS_BAR_COLOR.r,BOSS_BAR_COLOR.g,BOSS_BAR_COLOR.b,255);
			SDL_Rect insideBar = BOSS_BAR_DEST1;
			insideBar.h -= (1-ratio)*BOSS_BAR_DEST1.h;
			insideBar.y += (1-ratio)*BOSS_BAR_DEST1.h;
			SDL_RenderFillRect(renderer, &insideBar);
			insideBar.x = BOSS_BAR_DEST2.x;
			SDL_RenderFillRect(renderer, &insideBar);
		}
		...
		SDL_Rect src = BOSS_BAR_SRC;
		SDL_Rect dest = BOSS_BAR_DEST;
		SDL_RenderCopy(renderer, textures[SH_BOSS_BAR], &src, &dest);
	}
...
*/
	...
	int moveEnemy(Enemy **enemies, int * nbEnemy, int iEnemy){
		...
		else if((*enemies)[iEnemy].id == ARM_BOSS_ENEMY || (*enemies)[iEnemy].id == ARM_ENEMY){//arm_boss_appear
			(*enemies)[iEnemy].y += abs(SPAWN_ENEMY_Y[(*enemies)[iEnemy].id] - TARGET_ENEMY_Y[(*enemies)[iEnemy].id])/(float)(FRAME_BAR_SPAWN+EXTRA_TIME_WAIT_BOSS);
		}
	...
	int moveEnemyPart(EnemyPart **enemyParts, int * nbEnemyParts, int i){
	...
	void spawnEnemy(Enemy ** enemies, int *nbEnemy, int id, int nbSpawn, int* frameBossBar){
		*nbEnemy += nbSpawn;
		*enemies = realloc( *enemies, *nbEnemy * sizeof(Enemy));
		...
		for(int i = *nbEnemy-1, n=0; i > *nbEnemy - 1 - nbSpawn; i--, n++){
			if(id == ARM_ENEMY)
				dir *= -1;
			...
			(*enemies)[i] = (Enemy){
				...
				0, // framehit
				0, //dist
				FRAME_MULTI_SPAWN[id] * n,//frameWait
				dir, //dir
			...
			for(int j=0; j<= (*enemies)[i].nbWeapon; j++){
				...
				if(RELOAD_FRAME_RAND_ENEMY[(*enemies)[i].id][j] > 0)
					(*enemies)[i].weapons[j].frame_reload +=  FRAME_MULTI_SPAWN[id] * n + rand()%RELOAD_FRAME_RAND_ENEMY[(*enemies)[i].id][j];
				else
			...
			if(TYPE_MOVE_ENEMY[id] == INTERPOLATION){
				...
				do{
					...
					printf("retry\n");
				}while( (*enemies)[i].y + ENEMY_SRC[(*enemies)[i].id].h / RATIO_SIZE_ENEMY[(*enemies)[i].id] > 0);
				printf("\n\n" );
			}
			...
			//boss?
			if(id == ARM_BOSS_ENEMY || id == ARM_ENEMY){
				(*enemies)[i].frameWait = FRAME_BAR_SPAWN+EXTRA_TIME_WAIT_BOSS;
			}
		}

		//boss?
		if(id == ARM_BOSS_ENEMY){
			*frameBossBar = FRAME_BAR_SPAWN;
		}
	}
	...
	//   BOUCLE DU JEU  //``
	 // // // // // // //
		...
		// // // // //
			if(nbEnemy == 0){
	            //spawn
				int rng = BOSS_ENEMY;//rand()%(NB_ENEMY-2);
				if(rng == ARM_ENEMY){
					spawnEnemy(&enemies, &nbEnemy, ARM_BOSS_ENEMY, 1, &frameBossBar);
					spawnEnemy(&enemies, &nbEnemy, ARM_ENEMY, 2, &frameBossBar);
				}
				else{
					spawnEnemy(&enemies, &nbEnemy, rng, 2, &frameBossBar);
				}
			...
			for(int i=0; i<nbEnemyParts; i++)
				if(moveEnemyPart(&enemyParts, &nbEnemyParts, i))
					i--;
		...
		// Draw //`
		 // // //
			...
			for(int i=nbEnemyParts-1; i>=0; i--)
			...
			drawWarning(renderer, frameBossBar, textures);
			...
			drawBossBar(renderer, frameBossBar, findBoss(enemies, nbEnemy), textures, ratioWindowSize);
