...
void personnage(SDL_Renderer *renderer,SDL_Texture *texture_birds, int upscale, int animate_perso){
	...
	SDL_Rect target = {WINDOW_L/2 - ( (DIM_PERSO.x*upscale)/2 ),WINDOW_H/2,DIM_PERSO.x*upscale,DIM_PERSO.y*upscale};
	SDL_RenderCopy(renderer,texture_birds,&personnage,&target);
}

void score_print(SDL_Renderer *renderer, TTF_Font *myFont, int score){

	char scr[8];
	sprintf(scr, "%d", score);

	SDL_Surface* surfaceMessage_white = NULL;
	SDL_Surface* surfaceMessage_black = NULL;
	SDL_Texture* Message = NULL;

	SDL_FreeSurface(surfaceMessage_white);
	SDL_FreeSurface(surfaceMessage_black);

	surfaceMessage_white = TTF_RenderText_Blended(myFont, scr, WHITE);
	surfaceMessage_black = TTF_RenderText_Blended(myFont, scr, BLACK);

	SDL_DestroyTexture(Message);
  Message = SDL_CreateTextureFromSurface(renderer, surfaceMessage_black);

//A faire à chaque frame où on veut afficher le nombre
  SDL_Rect target = {WINDOW_L/2,WINDOW_H/6,0,0};
	target.w = (WINDOW_L/15)/2*strlen(scr);
	target.h = WINDOW_H/10;
	target.x -= target.w/2;

	SDL_Rect target_font_black = target;

	target_font_black.x -= target.x/100;
	target_font_black.y -= target.y/20;
	target_font_black.w += target.w/4;
	target_font_black.h += target.h/6;

  SDL_RenderCopy(renderer, Message, NULL, &target_font_black);
	SDL_DestroyTexture(Message);
  Message = SDL_CreateTextureFromSurface(renderer, surfaceMessage_white);
...
void view(SDL_Renderer * renderer, TTF_Font *myFont, int upscale, struct Environement environement[] ,int score,int animate_perso,int animate_sol,SDL_Texture *texture_background, SDL_Texture *texture_pipes, SDL_Texture *texture_birds, SDL_Texture *texture_medals, SDL_Texture *texture_scoreBoard, SDL_Texture *texture_sol){
	...
	score_print(renderer,myFont,score);
...
int flappy_bird(SDL_Window *window, SDL_Renderer *renderer, int upscale){
	...
	while(!end){
		...
		view(renderer,myFont,upscale,environement, score ,animate_perso++ , animate_sol++  ,texture_background,
																																									 texture_pipes,
																																									 texture_birds,
																																									 texture_medals,
																																									 texture_scoreBoard,
