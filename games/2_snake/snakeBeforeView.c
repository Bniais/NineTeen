...
void rm_body(SnakePart **snake, size_t *size, int nbRm, Vector2f* pastBody, SnakePart **deadBodies, size_t *nbDeadBodies, int deadly, int ratioTexture){
		...
		*deadBodies = realloc(*deadBodies, *nbDeadBodies * sizeof(SnakePart));
		for( int i = 0; i < nbRm; i++ ){
			(*deadBodies)[*nbDeadBodies - nbRm + i]=(SnakePart){
					(*snake)[*size - nbRm + i].x,
					(*snake)[*size - nbRm + i].y,
					FRUIT_TTL + ((i % ratioTexture)?NB_FRAME_DEATH_BODY:0),
...
void afficher_body(SDL_Renderer *renderer, SDL_Texture *snakeTexture, SnakePart *snake, size_t size, int frameUnkillable){
...
int tooCloseFromCorner(Fruit fruit){
	return (norme((Vector2f){fruit.dest.x - 0, fruit.dest.y - 0}) < DIST_CORNER_FRUIT + FRUIT_PROPRIETES[fruit.id][RADIUS] + (fruit.giant * GIANT_SIZE)
		|| norme((Vector2f){fruit.dest.x - SNAKE_PLAYGROUND_SIZE_W, fruit.dest.y - 0}) < DIST_CORNER_FRUIT + FRUIT_PROPRIETES[fruit.id][RADIUS] + (fruit.giant * GIANT_SIZE)
		|| norme((Vector2f){fruit.dest.x - SNAKE_PLAYGROUND_SIZE_W, fruit.dest.y - SNAKE_PLAYGROUND_SIZE_H}) < DIST_CORNER_FRUIT + FRUIT_PROPRIETES[fruit.id][RADIUS] + (fruit.giant * GIANT_SIZE)
		|| norme((Vector2f){fruit.dest.x - 0, fruit.dest.y - SNAKE_PLAYGROUND_SIZE_H}) < DIST_CORNER_FRUIT + FRUIT_PROPRIETES[fruit.id][RADIUS] + (fruit.giant * GIANT_SIZE));
}
...
void spawnFruit(SnakePart head, Fruit *fruit, int rang, int nbFruits, int nbFruitEaten, Fruit *bonus, Vector2f spawn, int spawnCondition, int retardFrame){
	...
	if( spawnCondition == FROM_NATURAL ){
		...
		if( i == MAX_TRIES_RAND ){
			i = 0;
			do{
				fruit[rang].dest.x = rand() % (int)(SNAKE_PLAYGROUND_SIZE_W);
				fruit[rang].dest.y = rand() % (int)(SNAKE_PLAYGROUND_SIZE_H);
				i++;
...
void spawnBonus(SnakePart head, Fruit* bonus, int nbFruitEaten, Fruit* fruit, int nbFruits){
	...
	i = -1;
	do{
		bonus->dest.x = rand() % (int)(SNAKE_PLAYGROUND_SIZE_W);
		bonus->dest.y = rand() % (int)(SNAKE_PLAYGROUND_SIZE_H);
		i++;
...
int scoreSize(Fruit fruit){
	...
	printf("score : %f\n",FRUIT_PROPRIETES[fruit.id][SCORE] );
...
void eat_fruit(SnakePart **snake, size_t *size, Fruit** fruitTab, size_t* nbFruits, Fruit fruit, float *speed, int *score, int *nbFruitEaten, Vector2f* pastBody, Fruit* bonus, SnakePart **deadBodies, size_t *nbDeadBodies, int* nbPotion, float* poisoned, Score** scoreAffichage, size_t* nbScoreAffichage){
	...
	*score += (fruit.giant == 0 ? 1 : GIANT_SCORE) * FRUIT_PROPRIETES[fruit.id][SCORE];
	...
	if( fruit.id >= NB_FRUITS ){
		switch ( fruit.id ) {
			case BOMBE:
				rm_body(snake, size, FLAT_RM_BOMB + RELATIVE_RM_BOMB * (*size - SIZE_PRE_RADIUS), pastBody, deadBodies, nbDeadBodies, 1, BOMB_TEXTURE_RATE);
				break;
			...
			case ARC_EN_CIEL:
				...
				*nbFruits += NB_SPAWN_ARC_EN_CIEL;
				...
				Vector2f corner;
				if( fruit.y > SNAKE_PLAYGROUND_SIZE_H/2 )
					if( fruit.x > SNAKE_PLAYGROUND_SIZE_W/2 )
						corner = (Vector2f){SNAKE_PLAYGROUND_SIZE_W, 0};
					else
						corner = (Vector2f){0, 0};
				else{
					if( fruit.x > SNAKE_PLAYGROUND_SIZE_W/2 )
						corner = (Vector2f){SNAKE_PLAYGROUND_SIZE_W, SNAKE_PLAYGROUND_SIZE_H};
					else
						corner = (Vector2f){0, SNAKE_PLAYGROUND_SIZE_H};
				}
...
void afficherScore(SDL_Renderer *renderer, Score scoreAffichage, TTF_Font *font ){
	...
	SDL_FreeSurface(surfaceMessage); //Pas sûr que ce soit obligatoire
	SDL_DestroyTexture(Message); //Pas sûr que ce soit obligatoire
}

// int launchSnake(SDL_Window *myWindow, SDL_Renderer* renderer, char *identifiant, char *token, int difficulte){
int main(){
	myInit();
	...
	//Fonts
	TTF_Font* scoreFont = TTF_OpenFont("./flappy.ttf", OPEN_FONT_SIZE);
	if( scoreFont == NULL ){
	...
	//Window and renderer
	SDL_Window *myWindow = SDL_CreateWindow("Snake", WINDOW_X, WINDOW_Y, SNAKE_PLAYGROUND_SIZE_W + 2 * HUD.x, SNAKE_PLAYGROUND_SIZE_H + 2 * HUD.y, WINDOW_FLAG);
	if( myWindow == NULL ){
	...
	SDL_Rect playgroundView = {HUD.x, HUD.y, SNAKE_PLAYGROUND_SIZE_W, SNAKE_PLAYGROUND_SIZE_H};
	SDL_Rect hudView = {WINDOW_X, WINDOW_Y, SNAKE_PLAYGROUND_SIZE_W + 2 * HUD.x, SNAKE_PLAYGROUND_SIZE_H + 2 * HUD.y};
	...
	if( snakeTexture == NULL  ||  fruitTexture == NULL || spawnTexture == NULL ){
	...
	while( !done ){
		...
		if( poison >= 1){
			...
			rm_body(&snakeBody, &snakeSize, (int)poison, pastBody, &deadBodies, &nbDeadBodies, 1, 1);
			poison -= (int)poison;
		...
		for( int i = 0; i<nbFruits; i++ ){
			if( fruit[i].hitbox == HIT ){
				...
				eat_fruit(&snakeBody, &snakeSize, &fruit, &nbFruits, fruit[i], &speedSnake, &score, &nbFruitEaten, pastBody, &bonus, &deadBodies, &nbDeadBodies, &nbPotion, &poisoned, &scoreAffichage, &nbScoreAffichage);
		...
		if( bonus.hitbox == HIT ){
			...
			eat_fruit(&snakeBody, &snakeSize, &fruit, &nbFruits, bonus, &speedSnake, &score, &nbFruitEaten, pastBody, &bonus, &deadBodies, &nbDeadBodies, &nbPotion, &poisoned, &scoreAffichage, &nbScoreAffichage);
		...
		//Snake
		afficher_body(renderer, snakeTexture, snakeBody, snakeSize, frameUnkillable);
		...
		if(bonus.frame < FRUIT_TTL + NB_FRAME_DEATH_FRUIT)
			afficher_fruit(renderer, fruitTexture, spawnTexture, bonus);
		...
		SDL_Rect hud = {HUD.x, HUD.y, SNAKE_PLAYGROUND_SIZE_W, SNAKE_PLAYGROUND_SIZE_H};
