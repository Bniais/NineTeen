...
void score_print(SDL_Renderer *renderer, TTF_Font *myFont, int score, SDL_Color color, int background){
	...
	surfaceMessage = TTF_RenderText_Blended(myFont, scr, color);

//A faire à chaque frame où on veut afficher le nombre
  SDL_Rect target = {WINDOW_L/2 -background ,WINDOW_H/6 -background,0,0};
	target.w = (WINDOW_L/15)/2*strlen(scr);
	target.h = WINDOW_H/10 ;
	target.x -= target.w/2;

  Message = SDL_CreateTextureFromSurface(renderer, surfaceMessage);
	SDL_RenderCopy(renderer, Message, NULL, &target);

	//libration de la memoire
...
void print_score(SDL_Renderer *renderer ,int score, int upscale)
{
	score = inverser_nombre(score);
	int sortie;

	SDL_Point chiffre = {POS_CHIFFRE.x,POS_CHIFFRE.y};
	SDL_Rect target = {WINDOW_L/2 -background ,WINDOW_H/6 -background , DIM_CHIFFRE.x * upscale, DIM_CHIFFRE.y * upscale};

	while( delete_last(&score,&sortie) )
	{
		chiffre.x = POS_CHIFFRE*sortie;
		target.x += DIM_CHIFFRE*upscale;
		SDL_RendererCopy(renderer,chiffre,&chiffre,&target);
	}


}

int inverser_nombre(int score)
{
	int reverse=0;
	while(score)
	{
...
int delete_last(int *score,int *sortie)
{
	if(*score)
	{
		*sorie = *score % 10;
...
void view(SDL_Renderer * renderer, TTF_Font *myFont, SDL_Point coordonner_perso,int upscale, struct Environement environement[] ,int score,int animate_perso,int animate_sol,SDL_Texture *texture_background, SDL_Texture *texture_pipes, SDL_Texture *texture_birds, SDL_Texture *texture_medals, SDL_Texture *texture_scoreBoard, SDL_Texture *texture_sol){
	...
	score_print(renderer,myFont,score, WHITE, 0);
...
int flappy_bird(SDL_Window *window, SDL_Renderer *renderer, int upscale){
	...
	while(!end){
		...
		view(renderer,myFont, coordonner_perso , upscale, environement, score ,animate_perso++ , animate_sol++  ,texture_background,
																																									 texture_pipes,
																																									 texture_birds,
																																									 texture_medals,
																																									 texture_scoreBoard,
	...
	while ( coordonner_perso.y < WINDOW_H - DIM_SOL.y*upscale )
	{
		...
		view(renderer,myFont, coordonner_perso , upscale, environement, score ,0 , 0  ,texture_background,
																																									 texture_pipes,
																																									 texture_birds,
																																									 texture_medals,
																																									 texture_scoreBoard,
