...
void obstacle(SDL_Renderer *renderer, SDL_Texture *texture_pipes, int upscale, int position){
...
void personnage(SDL_Renderer *renderer,SDL_Texture *texture_birds, int upscale, int animate){
...
void init_texture(SDL_Renderer *renderer ,SDL_Texture *texture_background, SDL_Texture *texture_pipes, SDL_Texture *texture_birds, SDL_Texture *texture_medals, SDL_Texture *texture_scoreBoard, SDL_Texture *texture_sol){

	texture_background = LoadTextureWithErrorCode(renderer,DIR_BACKGROUND);
	texture_pipes = LoadTextureWithErrorCode(renderer,DIR_PIPES);
	texture_birds = LoadTextureWithErrorCode(renderer,DIR_BIRDS);
	texture_medals = LoadTextureWithErrorCode(renderer,DIR_MEDALS);
	texture_scoreBoard = LoadTextureWithErrorCode(renderer,DIR_SCOREBOARD);
	texture_sol = LoadTextureWithErrorCode(renderer,DIR_SOL);
...
void view(SDL_Renderer * renderer,int animate, SDL_Texture *texture_background, SDL_Texture *texture_pipes, SDL_Texture *texture_birds, SDL_Texture *texture_medals, SDL_Texture *texture_scoreBoard, SDL_Texture *texture_sol){

	int upscale = WINDOW_H/DIM_BACKGROUND.y;

	background(renderer,texture_background,upscale);
	obstacle(renderer,texture_pipes,upscale,3); // 3 - 9 //
	sol(renderer,texture_sol,upscale);
	personnage(renderer,texture_birds,upscale,animate);
...
int flappy_bird(SDL_Window *window, SDL_Renderer *renderer){
	...
	SDL_Texture *texture_pipes;
	SDL_Texture *texture_birds;
	SDL_Texture *texture_medals;
	SDL_Texture *texture_scoreBoard;
	SDL_Texture *texture_sol;

	init_texture(renderer,texture_background,texture_pipes,texture_birds,texture_medals,texture_scoreBoard,texture_sol);

	for(int i = 0 ; i < 30 ; i++){
		SDL_RenderClear(renderer);
		if(i%3)
			view(renderer,i%3,texture_background,texture_pipes,texture_birds,texture_medals,texture_scoreBoard,texture_sol);
		else
			view(renderer,3,texture_background,texture_pipes,texture_birds,texture_medals,texture_scoreBoard,texture_sol);
