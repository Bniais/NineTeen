...
const int RATIO_ANIM_SOL = 20 * (FPS/30); // VITESSE DE DEPLACEMENT DU DECORS
...
// AFFICHAGE DU SOL
int sol(SDL_Renderer *renderer, SDL_Texture *texture_sol,int animate_sol)
{
	int error_code = 0;
	// ratio sol pour avoir 100 position de sol//
	int position_reference = -( (SOL.w*SCALE_TO_FIT)/100 )*animate_sol;

	SDL_Rect target = { position_reference,WINDOW_H-(SOL.h*SCALE_TO_FIT),SOL.w*SCALE_TO_FIT,SOL.h*SCALE_TO_FIT};
...
int personnage(SDL_Renderer *renderer,SDL_Texture *texture_birds, SDL_Point coordonner_perso, int animate_perso)
{
	...
	return SDL_RenderCopy(renderer,texture_birds,&pos_print,&target);
}
...
int view(SDL_Renderer * renderer, SDL_Point coordonner_perso, pilonne *pilonne ,int score,int animate_perso,int animate_sol,SDL_Texture *texture_background, SDL_Texture *texture_pipes, SDL_Texture *texture_birds, SDL_Texture *texture_medals, SDL_Texture *texture_scoreBoard, SDL_Texture *texture_sol, SDL_Texture *texture_chiffre)
{
	...
	if ( personnage(renderer , texture_birds, coordonner_perso  , animate_perso ) )
...
int traitement_pilonne(pilonne *pilonne, int traitement, int *score, long long *score_hash, long *const_1, long *const_2, long *const_3, long *const_4, Mix_Chunk *score_wav)
{
	...
	for(int i=0; i<PRELOAD_POS_OBSTACLE;i++)
		...
		pilonne[i].position -= SOL.w/RATIO_ANIM_SOL;

	if( pilonne[0].position + OBSTACLE_VERT.w*SCALE_TO_FIT < 0){
...
int flappy_bird(SDL_Window *window, SDL_Renderer *renderer)
{
	...
	if( !texture_background || !texture_pipes || !texture_birds || !texture_medals || !texture_scoreBoard || !texture_sol || !texture_chiffre)
	...
	if (!exit_code)
	{
		...
		// afficher la vue //
		exit_code = view(renderer, coordonner_perso , pilonne, score ,1 , 0 ,texture_background, 
																		   texture_pipes, 
																		   texture_birds, 
																		   texture_medals,  
																		   texture_scoreBoard, 
		...
		while(!end && !exit_code){
				...
				exit_code = view(renderer, coordonner_perso , pilonne, score ,animate_perso++ , animate_sol++  ,texture_background,
																												texture_pipes, 
																												texture_birds, 
																												texture_medals,  
																												texture_scoreBoard, 
			...
			int current_delay = times_at_end_frame - times_at_start_frame;
			if(current_delay < FRAME_TIME)
...
int main() {
	...
	printf("Code d'erreur de sorti : %d", flappy_bird(window, renderer) );
