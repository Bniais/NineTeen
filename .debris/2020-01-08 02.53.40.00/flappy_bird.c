...
const int RATIO_ANIM_SOL = 50; // plus c'est petit plus c'est rapide
...
void score_print(SDL_Renderer *renderer, int score){
	TTF_Init();
	TTF_Font *myFont = TTF_OpenFont("/Textures/flappy.ttf",100);
	char scr[8];
	sprintf(scr, "%d", score);

	SDL_Surface* surfaceMessage = TTF_RenderText_Blended(myFont, scr, WHITE);
