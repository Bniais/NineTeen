...
void sol(SDL_Renderer *renderer, SDL_Texture *texture_sol, int upscale,int animate_sol){
	...
	SDL_Rect target = {-animate_sol,WINDOW_H-(DIM_SOL.y*upscale),-animate_sol+DIM_SOL.x*upscale,DIM_SOL.y*upscale};
	SDL_RenderCopy(renderer,texture_sol,&sol,&target);


	while ( -animate_sol + target.x + DIM_SOL.x*upscale < WINDOW_L) {
		target.x += -animate_sol + DIM_SOL.x*upscale;
	  SDL_RenderCopy(renderer, texture_sol,&sol, &target);
	}
}
...
void view(SDL_Renderer * renderer,int animate_perso,int animate_sol,SDL_Texture *texture_background, SDL_Texture *texture_pipes, SDL_Texture *texture_birds, SDL_Texture *texture_medals, SDL_Texture *texture_scoreBoard, SDL_Texture *texture_sol){
...
int flappy_bird(SDL_Window *window, SDL_Renderer *renderer){
	...
	for(int i = 0 ; i < 100 ; i++){
		...
		if(i%3)
			view(renderer,i%3,i,texture_background,texture_pipes,texture_birds,texture_medals,texture_scoreBoard,texture_sol);
		else
			view(renderer,3,i,texture_background,texture_pipes,texture_birds,texture_medals,texture_scoreBoard,texture_sol);
