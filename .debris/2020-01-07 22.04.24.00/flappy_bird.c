...
// affichage du sol
void sol(SDL_Renderer *renderer, SDL_Texture *tilset, int upscale,int animate){
	SDL_Rect sol = {POS_SOL.x,POS_SOL.y,DIM_SOL.x,DIM_SOL.y};

	SDL_Rect target = {animate,WINDOW_H-(DIM_SOL.y*upscale),animate+DIM_SOL.x*upscale,DIM_SOL.y*upscale};
...
void view(SDL_Renderer * renderer,int animate, SDL_Texture *texture_background, SDL_Texture *texture_pipes, SDL_Texture *texture_birds, SDL_Texture *texture_medals, SDL_Texture *texture_scoreBoard, SDL_Texture *texture_sol){
	...
	sol(renderer,texture_sol,upscale,20);
