...
void personnage(SDL_Renderer *renderer,SDL_Texture *texture_birds, SDL_Point coordonner_perso,int upscale, int animate_perso){
	...
	SDL_Point center = { target.x + (target.x - target.w)/2  , target.y + (target.y - target.h)/2};
	printf("%d", SDL_RenderCopyEx(renderer,texture_birds,&personnage,&target,90,&center,SDL_FLIP_NONE) );
}
