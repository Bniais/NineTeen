...
const int RATIO_ANIM_SOL = 1; // plus c'est petit plus c'est rapide
...
void score_print(SDL_Renderer *renderer, TTF_Font *myFont, int score){
	...
	surfaceMessage = TTF_RenderText_Blended(myFont, scr, WHITE);
...
//A faire à chaque frame où on veut afficher le nombre
  SDL_Rect target = {WINDOW_L/2,WINDOW_H/6,0,0};
	...
	target_font_black.x -= target_font_black.x/10;
	target_font_black.y -= target_font_black.y/10;
	target_font_black.w += target_font_black.w/10;
	target_font_black.h += target_font_black.h/10;
