...
#define DISTANCE_UNDER_OBSTACLE 30
...
void view(SDL_Renderer * renderer,int preposition[] ,int animate_perso,int animate_sol,int ratio_pos_x,SDL_Texture *texture_background, SDL_Texture *texture_pipes, SDL_Texture *texture_birds, SDL_Texture *texture_medals, SDL_Texture *texture_scoreBoard, SDL_Texture *texture_sol){
	...
	for (int i = 0; i < PRELOAD_POS_OBSTACLE; i++) {
		obstacle(renderer,texture_pipes,upscale,preposition[i],ratio_pos_x); // 3 - 9 //
		ratio_pos_x+= DISTANCE_UNDER_OBSTACLE;
	}
...
int flappy_bird(SDL_Window *window, SDL_Renderer *renderer){
	...
	int preposition[PRELOAD_POS_OBSTACLE];
	for(int i=0; i<PRELOAD_POS_OBSTACLE;i++)
		preposition[i]= (rand() % (9 + 1 - 3)) + 3 ;
	...
	for(int i = 0,j=100; i < 100 ; i++,j--){
		...
		if(i%3)
			view(renderer,preposition,i%3,i,j,texture_background,texture_pipes,texture_birds,texture_medals,texture_scoreBoard,texture_sol);
		else
			view(renderer,preposition,3,i,j,texture_background,texture_pipes,texture_birds,texture_medals,texture_scoreBoard,texture_sol);
