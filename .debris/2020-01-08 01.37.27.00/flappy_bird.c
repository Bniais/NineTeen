...
int traitement_environement(struct Environement *environement, int upscale, int traitement, int *score, long long score_hash, int const_1, int const_2, int const_3, int const_4){
	...
	for(int i=0 ; i < PRELOAD_POS_OBSTACLE ; i++){
		if(environement[i].position + DIM_OBSTACLE.x*upscale < WINDOW_L/2 )
			if(environement[i+1].position + DIM_OBSTACLE.x*upscale > WINDOW_L/2)
				if(i!= traitement){
					printf("SCORE HASH = %lld\n SCORE RESULTAT = %lld",score_hash,hashage(*score, const_1, const_2, const_3, const_4));
