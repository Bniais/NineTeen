...
void background(SDL_Renderer *renderer, SDL_Texture* texture_background, int upscale){
	...
	SDL_Rect target = {0, 0, BACKGROUND.w*upscale, WINDOW_H};

	//On le dessine tant qu'on a pas rempli l'ecran
	while (target.x + BACKGROUND.w*upscale < WINDOW_L) {
		SDL_RenderCopy(renderer, texture_background, &BACKGROUND, &target);
		target.x += BACKGROUND.w*upscale;
	}
	SDL_RenderCopy(renderer, texture_background, &BACKGROUND, &target);
...
void obstacle(SDL_Renderer *renderer, SDL_Texture *texture_pipes, int upscale, int position, int position_axe_x){
	...
	SDL_Rect target = {position_axe_x,-TRANCHE_POS_OBSTACLE + (TRANCHE_POS_OBSTACLE/NB_POS_OBSTACLE)*position,OBSTACLE_VERT.w*upscale,OBSTACLE_VERT.h*upscale};

	SDL_RenderCopy(renderer,texture_pipes,&obstacle,&target);

	target.y += OBSTACLE_VERT.h*upscale + DISTANCE_BETWEEN_OBSTACLE*upscale;
	obstacle.x -= obstacle.w;
	SDL_RenderCopy(renderer,texture_pipes,&obstacle,&target);
...
void sol(SDL_Renderer *renderer, SDL_Texture *texture_sol, int upscale,int animate_sol){
	...
	SDL_Rect target = {-animate_sol,WINDOW_H-(SOL.h*upscale),SOL.w*upscale,SOL.h*upscale};

	SDL_RenderCopy(renderer,texture_sol,&SOL,&target);

	while (target.x + SOL.w*upscale < WINDOW_L) {
		target.x +=  SOL.w*upscale;
	  SDL_RenderCopy(renderer, texture_sol, &SOL, &target);
	}
}
...
void personnage(SDL_Renderer *renderer,SDL_Texture *texture_birds, SDL_Point coordonner_perso,int upscale, int animate_perso){
	...
	SDL_Rect target = {coordonner_perso.x - ( PERSO.w*upscale ) / 2, coordonner_perso.y - ( PERSO.h *upscale ) / 2,
		 								PERSO.w*upscale , PERSO.h*upscale};
...
void print_score(SDL_Renderer *renderer , SDL_Texture *texture_chiffre,int score, int upscale)
{
	...
	SDL_Rect target = {WINDOW_L/2 - CHIFFRE.w*upscale ,WINDOW_H/6  , CHIFFRE.w * upscale, CHIFFRE.h * upscale};

	for(int i=0 ; i < taille; i++)
	{
		chiffre.x = CHIFFRE.w * (score%10);
		target.x -= CHIFFRE.w * upscale;
...
void view(SDL_Renderer * renderer, SDL_Point coordonner_perso,int upscale, Environement *environement ,int score,int animate_perso,int animate_sol,SDL_Texture *texture_background, SDL_Texture *texture_pipes, SDL_Texture *texture_birds, SDL_Texture *texture_medals, SDL_Texture *texture_scoreBoard, SDL_Texture *texture_sol, SDL_Texture *texture_chiffre){
...
void init_environement(Environement *environement, int upscale, int *animate_perso, int *animate_sol, int *end, int* dead, int *traitement){
...
int traitement_environement(Environement *environement, int upscale, int traitement, int *score, long long *score_hash, long *const_1, long *const_2, long *const_3, long *const_4, Mix_Chunk *score_wav){

	if(traitement == -1){
		if(environement[0].position + OBSTACLE_VERT.w*upscale < WINDOW_L/2 ){
			traitement = 0;
		...
		if(environement[1].position + OBSTACLE_VERT.w*upscale < WINDOW_L/2 ){
			traitement = 1;
		...
		if(environement[2].position + OBSTACLE_VERT.w*upscale < WINDOW_L/2 ){
			traitement = 3;
			if(changeProtectedVar(score_hash, score, (*score)+1, const_1, const_2, const_3, const_4))
				Mix_PlayChannel( -1, score_wav,0);
			/*else
	...
	if( environement[0].position + OBSTACLE_VERT.w*upscale < 0){
...
int collision(Environement *environement, SDL_Point coordonner_perso, int upscale, Mix_Chunk *hurt_wav){
	int collision_detecter = 0;

	// permet de dire quand on sur l'axe d'un obstacle et de qu'elle obstacle
	// + detection collision
	for (int i = 0; i < PRELOAD_POS_OBSTACLE; i++) {
		if( environement[i].position + PERSO.w*upscale > (  WINDOW_L/2 - ( (OBSTACLE_VERT.w*upscale)/2 ) ) && environement[i].position + PERSO.w*upscale< (  WINDOW_L/2 + ( (OBSTACLE_VERT.w*upscale)/2 ) )){

			int bas =( (  -TRANCHE_POS_OBSTACLE + (TRANCHE_POS_OBSTACLE/NB_POS_OBSTACLE)*environement[i].hauteur ) + OBSTACLE_VERT.h*upscale ) + PERSO.h*upscale/2;
			int haut = (  -TRANCHE_POS_OBSTACLE + (TRANCHE_POS_OBSTACLE/NB_POS_OBSTACLE)*environement[i].hauteur ) + OBSTACLE_VERT.h*upscale + DISTANCE_BETWEEN_OBSTACLE*upscale - PERSO.h*upscale/2;
...
// detection du sol
	if(coordonner_perso.y + PERSO.h*upscale > WINDOW_H - SOL.h*upscale){
		collision_detecter = 1;
	}
...
int flappy_bird(SDL_Window *window, SDL_Renderer *renderer, int upscale){
	...
	init_environement(environement,upscale,&animate_perso,&animate_sol,&end,&dead,&traitement);
	// end init Environement //

	// init personnage y //

	SDL_Point coordonner_perso = {WINDOW_L/2 - ( (PERSO.w*upscale)/2 ) , WINDOW_H/2};
	...
	while(!end){
		...
		if(!dead){
			...
			view(renderer, coordonner_perso , upscale, environement, score ,animate_perso++ , animate_sol++  ,texture_background, texture_pipes, texture_birds, texture_medals,  texture_scoreBoard, texture_sol, texture_chiffre);

			// traitement de mon environement
			traitement = traitement_environement(environement,upscale,traitement,&score,  &score_hash,  &const_1,  &const_2, &const_3,  &const_4, score_wav);

			traitement_variable(&animate_perso,&animate_sol);

			dead = collision(environement,coordonner_perso,upscale,hurt_wav);

		}
		else{ //animate death
			coordonner_perso.y += gravity_speed++;
			view(renderer, coordonner_perso , upscale, environement, score ,0 , 0  ,texture_background, texture_pipes, texture_birds,  texture_medals, texture_scoreBoard, texture_sol,  texture_chiffre );

			if(coordonner_perso.y >= WINDOW_H - SOL.h*upscale)
				end = SDL_TRUE;
		}
...
int main() {
	...
	int upscale = WINDOW_H/BACKGROUND.h;
